...
function FolderPath(value, sep) {
  sep = sep || '/job/';
  if (!(this instanceof FolderPath)) {
    return new FolderPath(value, sep);
  }
  this.sep = sep;
  if (Array.isArray(value)) {
    ...
    if (value.match('^https?:\/\/')) {
      this.value = parseName(value, sep);
    } else {
...
FolderPath.prototype.path = function() {
  ...
  return new RawParam(this.sep + this.value.map(encodeURIComponent).join(this.sep));
};

FolderPath.prototype.parent = function() {
  return new FolderPath(this.value.slice(0, Math.max(0, this.value.length - 1)), this.sep);
};
